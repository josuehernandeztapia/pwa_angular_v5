...
export function calculateETA(createdAt: string, currentStatus: DeliveryStatus): string {
  ...
  const totalTimelineDays = DELIVERY_FSM.reduce((acc, transition) => acc + (transition.estimatedDays || 0), 0);

  let accumulatedDays = 0;
  if (DELIVERY_FSM.length > 0 && currentStatus !== DELIVERY_FSM[0].from) {
    for (const transition of DELIVERY_FSM) {
      accumulatedDays += transition.estimatedDays || 0;
      if (transition.to === currentStatus) {
        break;
      }
    }
  }

  if (!Number.isFinite(accumulatedDays) || accumulatedDays < 0) {
    accumulatedDays = 0;
  }

  const remainingDays = Math.max(totalTimelineDays - accumulatedDays, 0);
  const eta = new Date(baseDate);
  eta.setDate(eta.getDate() + accumulatedDays + remainingDays);
