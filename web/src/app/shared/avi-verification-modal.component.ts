...
import { AfterViewInit, Component, ElementRef, EventEmitter, Input, OnDestroy, OnInit, Output, ViewChild, DestroyRef, PLATFORM_ID, Renderer2, RendererFactory2, inject } from '@angular/core';
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';
import { IconComponent } from '@shared/icon/icon.component';
import { IconName } from '@shared/icon/icon-definitions';

import { AviQuestionGeneratorService, MicroLocalQuestion } from '@feature-services/avi/avi-question-generator.service';
import { AviSimpleConfigService, SimpleAviQuestion } from '@feature-services/avi/avi-simple-config.service';
import { VoiceFraudAnalysis, VoiceFraudDetectionService, VoiceMetrics } from '@feature-services/avi/voice-fraud-detection.service';
...
export class AviVerificationModalComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  @ViewChild('micButton') micButtonRef?: ElementRef<HTMLButtonElement>;
  ...
  private setupFocusManagement(): void {
    ...
    const dialog = this.dialogContainerRef?.nativeElement;
    if (!dialog) {
      return;
    }

    this.releaseFocusTrap = this.focusTrap.trap(dialog);
    queueMicrotask(() => {
      const focusTarget = this.micButtonRef?.nativeElement ?? dialog;
      try {
        focusTarget.focus();
      } catch {
        /* no-op */
