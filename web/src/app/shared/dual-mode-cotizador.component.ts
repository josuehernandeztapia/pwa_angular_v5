...
import { resolveProductPackageKey } from '@services/utils/market-key.util';
...
export class DualModeCotizadorComponent implements OnInit {
  ...
  async loadPackageData() {
    ...
    try {
      const saleType: 'contado' | 'financiero' = this.client?.flow?.includes('VentaDirecta') ? 'contado' : 'financiero';
      const context = {
        market: this.selectedMarket as PolicyMarket,
        saleType,
        clientType: this.selectedClientType as PolicyClientType
      };

      if (!resolveProductPackageKey(context)) {
        this.toast.error('No se encontró paquete disponible para la combinación seleccionada.');
        return;
      }

      this.packageData = await firstValueFrom(this.cotizadorEngine.getProductPackageForContext(context));
