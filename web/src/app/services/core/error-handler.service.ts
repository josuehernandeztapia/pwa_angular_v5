...
export class ErrorHandlerService implements ErrorHandler {
  ...
  addError(error: AppError): void {
    ...
    if (error.type !== 'error') {
      timer(10000).subscribe(() => this.removeError(error.id));
    }
  }
  ...
  showSuccess(message: string, duration: number = 5000): void {
    ...
    timer(duration).subscribe(() => this.removeError(successError.id));
  }
  ...
  showInfo(message: string, duration: number = 7000): void {
    ...
    timer(duration).subscribe(() => this.removeError(infoError.id));
  }
  ...
  private getHttpErrorActions(error: HttpErrorResponse): ErrorAction[] {
    ...
    switch (error.status) {
      case 0:
        actions.push({
          label: 'Reintentar',
          action: () => safeWindow()?.location?.reload(),
          style: 'primary'
        });
        break;
      ...
      case 403:
        actions.push({
          ...
          action: () => {
            safeWindow()?.open('mailto:soporte@conductores.com', '_blank');
          },
          style: 'secondary'
        });
        break;
      ...
      case 504:
        actions.push({
          ...
          action: () => {
            safeWindow()?.open('mailto:soporte@conductores.com?subject=Error del Servidor', '_blank');
          },
          style: 'secondary'
        });
        break;
