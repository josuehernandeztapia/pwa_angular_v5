...
import { NotificationPayload, NotificationHistory } from '@interfaces/notification';
...
export class PushNotificationService {
  ...
  private checkSupport(): void {
    ...
    this.isSupported = this.swPush.isEnabled && !!win && 'Notification' in win;
    if (this.isSupported) {
  ...
  async subscribeToNotifications(): Promise<PushSubscription | null> {
    try {
      ...
      if (subscription) {
        ...
        storage?.setItem('push_subscription', JSON.stringify(savedSub));
  ...
  async unsubscribeFromNotifications(): Promise<boolean> {
    try {
      ...
      const storage = this.getStorage();
      storage?.removeItem('push_subscription');
  ...
  private loadExistingSubscription(): void {
    const storage = this.getStorage();
    const stored = storage?.getItem('push_subscription');
    if (stored) {
      try {
  ...
  private handleNotificationClick(event: any): void {
    ...
    const isKarma = Boolean(win && win.__karma__ !== undefined);

    switch (data.type) {
      case 'payment_due':
      case 'gnv_overage':
        if (data.payment_link) {
          safeWindow()?.open(data.payment_link, '_blank');
        }
        break;
      ...
      case 'document_pending':
        if (data.client_id) {
          ...
          if (!isKarma) {
            safeWindow()?.location.assign(`/clientes/${data.client_id}#documentos`);
          }
        }
        break;
      ...
      case 'contract_approved':
        if (data.client_id) {
          if (!isKarma) {
            safeWindow()?.location.assign(`/clientes/${data.client_id}`);
          }
        }
        break;
      ...
      default:
        if (data.action_url) {
          if (!isKarma) {
            safeWindow()?.location.assign(data.action_url);
          }
        }
    }
  ...
  private loadStoredNotifications(): void {
    const storage = this.getStorage();
    const stored = storage?.getItem('notification_history');
    if (stored) {
      try {
        const notifications = JSON.parse(stored);
        this.notifications$.next(notifications);
      } catch (error) {
        storage?.removeItem('notification_history');
      }
    }
  }
  ...
  private addNotificationToHistory(notification: NotificationHistory): void {
    ...
    const storage = this.getStorage();
    storage?.setItem('notification_history', JSON.stringify(updated));
  }
