...
export class AuthService {
  ...
  refreshToken(): Observable<AuthResponse> {
    ...
    const refreshToken = this.getStorage()?.getItem(this.refreshTokenKey);
  ...
  requestPasswordReset(email: string): Observable<boolean> {
    return of(null).pipe(
      ...
      map(() => {
        ...
        const storage = this.getStorage();
        const resetRequests = JSON.parse(storage?.getItem('passwordResetRequests') || '[]');
        resetRequests.push({
          email,
          requestedAt: new Date().toISOString()
        });
        storage?.setItem('passwordResetRequests', JSON.stringify(resetRequests));
  ...
  private setAuthData(authResponse: AuthResponse, rememberMe?: boolean): void {
    ...
    try {
      ...
      // Ignore storage quota issues in SSR / private mode
    }
    ...
    return of(null).pipe(
      ...
      map(() => {
        ...
        const pendingRegistrations = JSON.parse(
          this.getStorage()?.getItem('pendingRegistrations') || '[]'
        );
        pendingRegistrations.push(pendingRegistration);
        const storage = this.getStorage();
        storage?.setItem('pendingRegistrations', JSON.stringify(pendingRegistrations));

        console.info('[AuthService] Verification email simulated', {
          to: registrationData.email,
          subject: 'Verificar tu cuenta - Conductores PWA',
          verificationLink: `${(safeWindow()?.location?.origin ?? '')}/verify-email?token=${pendingRegistration.verificationToken}`
        });
  ...
  verifyEmail(token: string): Observable<{ message: string; success: boolean }> {
    return of(null).pipe(
      ...
      map(() => {
        ...
        const storage = this.getStorage();
        storage?.setItem('pendingRegistrations', JSON.stringify(updatedRegistrations));
