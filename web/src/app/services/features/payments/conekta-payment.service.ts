...
const getConekta = (): any => {
  const win = safeWindow() as any;
  if (win?.Conekta) {
    return win.Conekta;
  }
  return (globalThis as any).Conekta;
};
...
export class ConektaPaymentService {
  ...
  private retryDisabled: boolean = (() => {
    const win = safeWindow() as any;
    return Boolean(win && (win.__karma__ !== undefined || typeof win.jasmine !== 'undefined'));
  })();

  constructor(private http: HttpClient) {
    if (this.isBrowser) {
      Promise.resolve()
        .then(() => this.loadConektaSDK())
        .catch(() => {
          // SDK loading is best-effort; explicit calls will surface errors
        });
    }
  }
  ...
  private async loadConektaSDK(): Promise<void> {
    ...
    if (!this.isBrowser) {
      throw new Error('Conekta SDK requires a browser environment');
    }
    ...
    if (!loaded?.Token?.create) {
      throw new Error('Failed to initialize Conekta SDK');
    }
