...
export class CotizadorEngineService {
  ...
  resolvePackageKey(context: ProductPackageContext): string {
    const key = resolveProductPackageKey(context);
    if (!key) {
      throw new Error(`No se pudo resolver el paquete para el contexto: ${JSON.stringify(context)}`);
    }
    return key;
  }
  ...
  generateQuoteWithPackage(market: string, selectedOptionals: string[] = [], downPayment: number = 0, term: number = 12): Observable<Quote & { packageData: ProductPackage }> {
    return this.getProductPackage(market).pipe(
      ...
      map((packageData: ProductPackage) => {
        ...
        const businessFlow = this.getBusinessFlowFromMarket(market);
        ...
        return {
          ...
          term: validatedTerm,
          monthlyPayment,
          market: normalizedMarket as Market,
