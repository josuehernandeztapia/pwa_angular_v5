...
import { AVIDualEngineService, DualEngineResult } from '@feature-services/avi/avi-dual-engine.service';
import { AVIConfusionMatrixService, CalibrationSample } from '@feature-services/avi/avi-confusion-matrix.service';
...
export class AVICalibrationService {
  ...
  private addMockSamplesSequentially(samples: Array<Omit<CalibrationSample, 'id' | 'timestamp'>>, index: number): Observable<boolean> {
    ...
    return this.confusionMatrixService.addCalibrationSample(samples[index]).pipe(
      switchMap(() =>
        timer(50).pipe(
          switchMap(() => this.addMockSamplesSequentially(samples, index + 1))
        )
      )
    );
  }
