...
import { Observable, BehaviorSubject, Subject, of, throwError, interval, timer, from, EMPTY } from 'rxjs';
import { catchError, map, delay, startWith, switchMap, takeUntil } from 'rxjs/operators';
import { environment } from '@environments/environment';
import { safeDispatchEvent } from '@services/utils/ssr/dom-events.util';
...
export class VoiceValidationService {
  ...
  private pollProcessingResults(processingId: string, sessionId: string): void {
    ...
    interval(2000)
      .pipe(
        ...
        takeUntil(stop$),
        switchMap(() =>
          from(this.getProcessingResult(processingId)).pipe(
            catchError(error => {
  ...
  private readonly API_CONFIG = {
    ...
    MOCK_MODE: environment.features.enableMockData ?? false,
    LOGGING_ENABLED: !environment.production
  };
