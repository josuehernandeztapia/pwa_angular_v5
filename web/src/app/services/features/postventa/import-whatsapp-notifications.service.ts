...
import { WhatsappService } from '@feature-services/postventa/whatsapp.service';
import { IntegratedImportStatus, ImportMilestoneEvent } from '@feature-services/postventa/integrated-import-tracker.service';
...
export class ImportWhatsAppNotificationsService {
    ...
    return this.getNotificationSettings(clientId).pipe(
      ...
      map(settings => {
        const templateData = {
          ...
          tracking_url: `${this.getAppOrigin()}/tracking/client/${clientId}`,
  ...
  private buildNotificationTemplate(
    ...
    return {
      ...
      tracking_url: `${this.getAppOrigin()}/tracking/client/${settings.clientId}`,
  ...
  private getAppOrigin(): string {
    try {
      const globalRef = typeof globalThis !== 'undefined' ? (globalThis as { location?: { origin?: string } }) : undefined;
      const origin = globalRef?.location?.origin;
      if (origin && origin.length > 0) {
        return origin;
      }
    } catch {
      // Ignore access errors and fall through to default origin.
    }

    return 'https://app.conductores.mx';
  }
