...
import { ContractService } from '@feature-services/contracts/contract.service';
import { DeliveriesService } from '@feature-services/logistics/deliveries.service';
import { ImportWhatsAppNotificationsService } from '@feature-services/postventa/import-whatsapp-notifications.service';
import { WhatsAppNotificationResult } from '@interfaces/notification';
import { VehicleAssignmentRequest, VehicleAssignmentService } from '@feature-services/clients/vehicle-assignment.service';
import { ImportMilestoneEvent, ImportTrackerMetrics, IntegratedImportStatus } from './integrated-import.types';
...
export { EnhancedImportStatusResponse, ImportMilestoneEvent, ImportTrackerMetrics, IntegratedImportStatus } from './integrated-import.types';
...
export class IntegratedImportTrackerService {
  ...
  private readonly baseUrl = resolveBffBaseUrl();
  ...
  private fetchImportStatus(clientId: string): Observable<IntegratedImportStatus | null> {
    return this.http
      .get<IntegratedImportStatus>(`${this.baseUrl}/v1/clients/${clientId}/import-status`)
      .pipe(
  ...
  private getContractsWithVehicles(clientId: string): Observable<any[]> {
    return this.http
      .get<any[]>(`${this.baseUrl}/v1/clients/${clientId}/contracts-vehicles`)
      .pipe(catchError(() => of([])));
  }
  ...
  getIntegratedImportStatus(clientId: string): Observable<IntegratedImportStatus | null> {
    return combineLatest([
      this.fetchImportStatus(clientId),
      ...
      map(([importStatus, triggers, deliveryOrders]) => {
        ...
        const integrated: IntegratedImportStatus = {
          ...
          deliveryOrderId: latestDeliveryOrder?.id ?? importStatus.deliveryOrderId,
          contractId: latestTrigger?.contractId ?? importStatus.contractId,
          syncStatus: this.determineSyncStatus(importStatus, deliveryOrders, triggers),
          lastSyncDate: importStatus.lastSyncDate ?? new Date(),
          estimatedDeliveryDate:
            importStatus.estimatedDeliveryDate ?? this.calculateEstimatedDeliveryDate(importStatus),
          actualDeliveryDate:
            importStatus.actualDeliveryDate ?? (latestDeliveryOrder?.updatedAt ? new Date(latestDeliveryOrder.updatedAt) : undefined)
        };
  ...
  updateImportMilestone(
    ...
    return this.http.patch<ImportStatus>(`${this.baseUrl}/v1/clients/${clientId}/import-status`, {
      ...
      map((patchedStatus: ImportStatus) => ({
        ...patchedStatus,
        ...
        if (deliveryStatus && deliveryOrder.status !== deliveryStatus) {
          ...
          if (event) {
            this.deliveriesService.transition(deliveryOrder.id, {
              event: event as any,
              meta: {
                milestone: milestone as keyof ImportStatus,
  ...
  private mapImportMilestoneToDeliveryStatus(
    ...
    const key = milestone as keyof typeof mapping;
    return mapping[key]?.[status] ?? null;
  }
  ...
  private recordMilestoneEvent(
    ...
    const event: ImportMilestoneEvent = {
      id: `milestone-${Date.now()}-${clientId}-${String(milestone)}`,
      clientId,
      milestone: milestone as keyof ImportStatus,
    ...
    return this.getClientData(clientId).pipe(
      switchMap(clientData =>
        this.importWhatsAppService.sendMilestoneNotification(
          clientId,
          milestone as keyof ImportStatus,
          status,
          importStatus,
          clientData
        )
  ...
  getIntegratedSummary(clientId: string): Observable<{
    ...
    return combineLatest([
      ...
      map(([importStatus, contractsWithVehicles]) => ({
        ...
        assignmentConsistency: this.validateAssignmentConsistency(importStatus, contractsWithVehicles || [])
      }))
    );
  }
