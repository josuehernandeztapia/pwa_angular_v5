...
let AGUASCALIENTES_FINANCIERO_DOCS: Document[] = [];
let EDOMEX_MIEMBRO_DOCS: Document[] = [];
let EDOMEX_AHORRO_DOCS: Document[] = [];
let COLLECTIVE_CREDIT_CLIENTS: Client[] = [];
let INITIAL_CLIENTS: Client[] = [];
...
if (environment.features.enableMockData) {
  ...
  COLLECTIVE_CREDIT_CLIENTS = Array.from({ length: 12 }, (_, i) => ({
    id: `cc-${i + 1}`,
  ...
  INITIAL_CLIENTS = [
    ...
    },
    {
      ...
      documents: EDOMEX_AHORRO_DOCS.map(doc => ({ ...doc, status: DocumentStatus.Aprobado })),
      events: [{ id: 'evt4-1', timestamp: new Date(), message: '¡Meta de ahorro completada! Listo para iniciar Venta a Plazo.', actor: Actor.Sistema, type: EventType.GoalAchieved }],
      ecosystemId: 'eco-2'
    },
    {
      id: 'laura-1',
...
export class ClientDataService {
  ...
  updateDocumentStatus(
    ...
    const updatedDocuments = client.documents.map(doc =>
      doc.id === documentId ? { ...doc, status, ...changes } : doc
    );
  ...
  deleteClient(clientId: string): Observable<boolean> {
    const existed = this.clientsDB.delete(clientId);
    if (existed) {
      this.clientsSubject.next(Array.from(this.clientsDB.values()));
    }
