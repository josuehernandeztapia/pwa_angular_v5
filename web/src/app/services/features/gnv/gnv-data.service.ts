...
export class GnvDataService {
  ...
  getEcosystemGNVReport(ecosystemId: string, month: string): Observable<{
    ...
    const fallback = this.getMockEcosystemReport(ecosystemId, month);

    return this.http.get<typeof fallback>(`${this.baseUrl}/gnv/ecosystems/${ecosystemId}/report/${month}`, {
      headers: this.getHeaders()
    }).pipe(
      retry(2),
  ...
  getGNVDashboardStats(): Observable<{
    ...
    const fallback = this.getMockDashboardStats();

    return this.http.get<typeof fallback>(`${this.baseUrl}/gnv/dashboard/stats`, {
      headers: this.getHeaders()
    }).pipe(
      retry(2),
      catchError(() => of(fallback))
  ...
  searchConsumption(filters: {
    ...
    return this.http.post<typeof fallback>(`${this.baseUrl}/gnv/consumption/search`, filters, {
      headers: this.getHeaders()
    }).pipe(
      retry(2),
      catchError(() => of(fallback))
    ...
    return this.getDriverConsumption(driverId).pipe(
      map(consumptions => {
        const stationUsage = consumptions.reduce((acc, consumption) => {
          const current = acc[consumption.station_id] ?? {
            station_id: consumption.station_id,
            station_name: consumption.station_name,
            usage_count: 0
          };
          current.usage_count++;
          acc[consumption.station_id] = current;
          return acc;
        }, {} as Record<string, { station_id: string; station_name: string; usage_count: number }>);

        const total = consumptions.length || 1;
