...
export class DeliveriesService {
  ...
  /**
   * Sync existing delivery data to NEON database through a bulk endpoint.
   * This is used during migration or when rehydrating the reporting store.
   */
  syncDeliveriesToNeon(deliveries: DeliveryOrder[]): Observable<{ synced: number; errors: number }> {
    if (!deliveries.length) {
      return of({ synced: 0, errors: 0 });
    }

    this.logger.log(`Syncing ${deliveries.length} deliveries to NEON database...`);

    return this.http
      .put<DeliveryBulkSyncResponse>(`${this.baseUrl}/v1/deliveries/sync`, { deliveries })
      .pipe(
        map(response => {
          const failures = response.failures ?? [];
          if (failures.length) {
            this.logger.warn(' Some deliveries failed to sync with NEON', failures);
          }

          const skipped = response.skipped ?? 0;
          const failed = response.failed ?? failures.length;
          const synced = response.synced ?? Math.max(0, deliveries.length - failed - skipped);

          if (response.metadata?.processedAt) {
            this.logger.log(
              ` Delivery bulk sync processed at ${response.metadata.processedAt} ` +
              `(duration: ${response.metadata.durationMs ?? 'n/a'} ms)`
            );
          }
        ...
        catchError(error => {
          this.logger.error(' Bulk delivery sync to NEON failed', error);
          return of({ synced: 0, errors: deliveries.length });
        })
      );
  }
...
interface DeliveryBulkSyncFailure {
  id: string;
  reason: string;
  statusCode?: number;
...
interface DeliveryBulkSyncResponse {
  synced: number;
  failed: number;
  skipped?: number;
  metadata?: {
  ...
  failures: DeliveryBulkSyncFailure[];
}
